    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-navy">
        <div className="text-center">
          ...
          <p className="text-slate-450 font-bold">বইয়ের তালিকা লোড হচ্ছে...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-20">
      ...
      <div className="flex justify-between items-center mb-10 no-print">
        <button 
          ...
          className="flex items-center space-x-2.5 px-6 py-3.5 bg-slate-900 border border-slate-850 text-slate-300 rounded-[24px] font-black text-xs hover:bg-slate-800 hover:text-white transition-all shadow-sm active:scale-95 duration-200 cursor-pointer"
        >
      ...
      <div className="text-center mb-16 relative no-print">
        ...
        <div className="inline-flex items-center space-x-3 px-5 py-2.5 bg-slate-950/40 border border-slate-800/80 rounded-full text-[11px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-8">
          <TrendingUp className="w-4 h-4" />
          <span>Research & Digital Archive</span>
        </div>
        <h1 className="text-5xl md:text-7xl font-black text-white mb-8 font-sans tracking-tight leading-[1.1]">
          বইয়ের <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-indigo-300">ডিজিটাল ক্যাটালগ</span>
        </h1>
        <p className="text-slate-450 max-w-2xl mx-auto text-lg leading-relaxed font-semibold">ডিপার্টমেন্ট অফ ইকোনমিক্স ডিজিটাল লাইব্রেরি। আপনার প্রয়োজনীয় ক্যাটালগটি খুঁজে নিন এবং সংগ্রহের সমৃদ্ধি বাড়ান।</p>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === tab.id 
                ? "bg-indigo-600 text-white shadow-2xl shadow-indigo-950/40" 
                : "bg-slate-900 border border-slate-850 text-slate-400 hover:bg-slate-850 hover:text-white"
            )}
          >
      ...
      <div className="max-w-5xl mx-auto mb-10 text-center no-print-area no-print">
        <p className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest mb-3.5">দ্রুত ক্যাটালগ আবিষ্কার করুন</p>
        <div className="flex flex-wrap items-center justify-center gap-2 max-w-4xl mx-auto">
          <button
            ...
            className={cn(
              "px-4 py-2 rounded-full text-xs font-black transition-all duration-200 active:scale-95 border border-slate-850",
              activeTab === 'categories' && selectedCategory === 'all'
                ? "bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-950/20"
      ...
      <div className="max-w-4xl mx-auto mb-24 space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 group w-full">
            <Search className="absolute left-7 top-1/2 -translate-y-1/2 w-6 h-6 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
            <input 
              type="text" 
              placeholder="বইয়ের নাম, লেখক বা ক্যাটাগরি দিয়ে খুঁজুন..."
              className="w-full pl-20 pr-8 py-6 bg-slate-900 border-2 border-slate-850 rounded-[40px] focus:outline-none focus:ring-8 focus:ring-indigo-500/5 focus:border-indigo-500 focus:text-white transition-all shadow-xl font-bold text-lg text-slate-200 placeholder:text-slate-500"
              value={searchTerm}
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto">
            <button 
              ...
              className={cn(
                "p-6 rounded-[35px] border border-slate-850 bg-slate-900 text-slate-400 transition-all shadow-xl active:scale-95 flex items-center justify-center hover:bg-slate-850",
                showFilters ? "bg-indigo-650 border-indigo-650 text-white" : ""
              )}
            >
            ...
            <section key={cat} className="group/section">
              <div className="flex justify-between items-end mb-6 px-4">
                <div className="space-y-1">
                  <h2 className="text-2xl font-black text-white group-hover/section:text-[#05d5a1] transition-colors uppercase tracking-tight">{cat}</h2>
                  <div className="w-12 h-1 bg-indigo-500 rounded-full group-hover/section:w-20 transition-all duration-500" />
                </div>
                <div className="flex gap-2">
                  <button onClick={() => {
                    const el = document.getElementById(`scroll-${groupIdx}`);
                    el?.scrollBy({ left: -300, behavior: 'smooth' });
                  }} className="p-3 bg-slate-900 rounded-xl hover:bg-slate-800 border border-slate-850 shadow-sm transition-all active:scale-90 cursor-pointer">
              ...
              <div className="relative group/shelf">
                ...
                >
                  {catBooks.map((book) => (
                    ...
                    >
                      <div className="group/card bg-slate-900 rounded-xl overflow-hidden border border-slate-850 shadow-sm hover:border-indigo-500/40 hover:scale-[1.04] hover:-translate-y-1.5 transition-all duration-300 p-1.5 h-full flex flex-col">
                        <div className="aspect-[3/4] relative overflow-hidden bg-slate-950 rounded-lg mb-2">
                          <img 
                        ...
                        <div className="px-1 pb-1 flex flex-col flex-1 text-left">
                          <h3 className="text-[10px] sm:text-[11px] font-black text-slate-200 group-hover/card:text-indigo-400 transition-colors mb-0.5 line-clamp-2 leading-tight min-h-[1.8rem]">{book.title}</h3>
                          <p className="text-[8px] sm:text-[9px] text-slate-400 mb-2 font-bold truncate opacity-80">{book.author}</p>
                          
                          <div className="mt-auto flex items-center justify-between gap-1">
                             <div className="text-[8px] sm:text-[9px] font-mono font-black text-slate-400 bg-slate-950 px-2 py-0.5 rounded-md border border-slate-850">
                                {book.isEBook ? 'Digital' : 'Hardcopy'}
                             </div>
                             <button 
                               onClick={() => setSelectedBook(book)}
                               className="p-1.5 bg-slate-900 border border-slate-800 text-slate-300 rounded-md hover:bg-indigo-650 hover:text-white hover:border-indigo-500 transition-all active:scale-95 cursor-pointer"
                             >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 px-4 text-left">
            {filteredBooks.map((book) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                className="group/card bg-slate-900 rounded-xl overflow-hidden border border-slate-850 shadow-sm hover:border-indigo-500/40 hover:scale-[1.04] hover:-translate-y-1.5 transition-all duration-300 p-1.5 flex flex-col"
              >
                <div className="aspect-[3/4] relative overflow-hidden bg-slate-950 rounded-lg mb-2">
                  <img 
                    src={book.cover || 'https://placehold.co/400x600/eee/999?text=Cover+Not+Found'}        
                ...
                <div className="px-1 pb-1 flex flex-col flex-1">
                  ...
                  <div className="mt-auto flex items-center justify-between gap-1">
                    <button 
                      onClick={() => setSelectedBook(book)}
                      className="w-full py-2 bg-slate-950 border border-slate-800 text-slate-300 rounded-md hover:bg-indigo-650 hover:text-white hover:border-indigo-500 transition-all text-[10px] font-bold cursor-pointer"
                    >
                      বিস্তারিত
                    </button>
                  </div>
      ...
      {filteredBooks.length === 0 && (
         <div className="text-center py-24 px-6 bg-slate-900/60 rounded-[80px] border-4 border-dashed border-slate-850 max-w-4xl mx-auto">
            <Bookmark className="w-16 h-16 text-slate-500 mx-auto mb-6" />
            <p className="text-slate-200 font-extrabold text-2xl mb-3">ক্যাটালগ বর্তমানে খালি রয়েছে</p>
            <p className="text-slate-450 font-medium max-w-lg mx-auto text-sm leading-relaxed mb-8">
